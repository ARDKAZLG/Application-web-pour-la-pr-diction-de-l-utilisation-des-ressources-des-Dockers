...
def create_containers(request):
    if request.method == 'POST':
        ...
        if port and name:
          command = f"docker run -it -d --name {name} -p {port}:80 {image}"
...
def generate_mem_chart(csv_file):
    ...
    image_path = 'static/dist/img/mem_chart.png'  
    plt.savefig(image_path, format='png')
    plt.close(fig)
...
def start_container(request, container_id):
    try:
        ...
        if result.returncode == 0:
            ...
            response = redirect(refresh_url)
        else:
            response = redirect('PerformancesDetails')
        ...
        return redirect('PerformancesDetails')  # Redirect to the details page on error
